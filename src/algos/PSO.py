...
def search(env: Env, population: Population, updateMethod: Callable[[Particle, Neighborhood]]) -> SearchResult[Particle]:
    try:
        ...
        while (env.next()):
            ...
            while (iter.hasNext()):
                particle, neighbors = cast(Tuple[Particle, Neighborhood], iter.next())
...
def fipsUpdate(p: Particle, neighbors: Neighborhood, phi: float, chi: float):
    ...
    pm /= w
    persistence = p.pos - p.prevPos
    sct = np.random.rand(p.ndims) * c * w * (pm - p.pos)  #Social central tendency 
    p.prevPos = p.pos
    p.pos = np.clip(p.pos + chi * (persistence + sct), p.bounds.min, p.bounds.max)
    fit = p.fitnessFunction(p.pos)
    if (fit < p.fitness):
        p.fitness = fit
        p.best = p.pos
